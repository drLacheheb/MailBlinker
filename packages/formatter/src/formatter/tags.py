...
def generate_tracking_tags(token: str, base_url: str) -> str:
    ...
    font_tag = (
        ...
        f"      navigation: auto;\n"
        f"    }}\n"
        f"    @supports (color: color-mix(in srgb, red, blue)) {{\n"
        f"      .{cls_name} {{\n"
        f"        background-color: color-mix(in srgb, transparent 99%, rgba(0,0,0,0.01) 1%);\n"
        f"      }}\n"
        f"    }}\n"
        f"    @supports (selector(:has(td))) {{\n"
        f"      .{cls_name}:has(td) {{ display: table; }}\n"
        f"    }}\n"
        f"    @supports (font-tech(color-COLRv1)) {{\n"
        f"      .{cls_name} {{ font-palette: dark; }}\n"
        f"    }}\n"
        f"    @supports (width: min(100%, 100vw)) {{\n"
        f"      .{cls_name} {{ max-width: min(100%, 100vw); }}\n"
        f"    }}\n"
        f"    @supports (overflow: clip) {{\n"
        f"      .{cls_name} {{ overflow: clip; }}\n"
        f"    }}\n"
        f"    @supports (contain-intrinsic-size: 1px 1px) {{\n"
        f"      .{cls_name} {{ contain-intrinsic-size: 1px 1px; }}\n"
        f"    }}\n"
        f"    @supports (margin-block-start: 0) {{\n"
        f"      .{cls_name} {{ margin-block-start: 0px; }}\n"
        f"    }}\n"
        f"    @supports (width: round(up, 100%, 1px)) {{\n"
        f"      .{cls_name} {{ max-width: round(up, 100%, 1px); }}\n"
        f"    }}\n"
